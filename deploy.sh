...
if [ -f "tpdb/settings.py" ]; then
    DJANGO_SETTINGS="tpdb.settings"
elif [ -f "tpdb/production_settings.py" ]; then
    DJANGO_SETTINGS="tpdb.production_settings"
else
...
CRON_JOB="* * * * * cd $PROJECT_DIR && $VENV_DIR/bin/python manage.py process_pending_imports --max-files 1 >> /var/log/tpdb/excel_imports.log 2>&1"
