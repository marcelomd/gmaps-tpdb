...
def get_columns(ws):
    ...
    for i, v in enumerate(row):
        if v is None:
            continue
        col_nums[v.strip().lower()] = i

    formula_mz = []
    for i in range(1, 11):
        formula_name = f"formula f{i} [m+h]"
        mz_name = f"m/z f{i}"
        # Special case for F2 which has different naming
        if i == 2:
            formula_name = "ion formula f2"
        formula = col_nums.get(formula_name)
        mz = col_nums.get(mz_name)
        formula_mz.append(FormulaMz(formula=formula, mz=mz))

    columns = Columns(
        name=col_nums.get("composto"),
        origin=col_nums.get("composto de origem"),
        clas=col_nums.get("classe"),
        subclass=col_nums.get("subclasse"),
        treatment=col_nums.get("tratamento"),
        type=col_nums.get("tipo de composto"),
        mode=col_nums.get("modo"),
        neutral=col_nums.get("formula neutra [m]"),
        mz=col_nums.get("m/z íon"),
        reference=col_nums.get("reference"),
        smile=col_nums.get("smile (fórmula neutra)"),
...
def process_pending(max_files=1):
    logger.info(f"process_pending nonce: {os.urandom(8)}")
