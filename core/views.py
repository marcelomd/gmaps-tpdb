...
def generate_molecule_image(smile_string, size=(300, 200)):
    """Generate a molecule image from a SMILE string"""
    if not RDKIT_AVAILABLE or not smile_string:
        return None
...
def serialize_compound(compound):
    ...
    return {
        ...
        } if compound.origin else None,
        'class': {
            'id': str(compound.clas.id),
            'name': compound.clas.name
        },
        'subclass': {
            'id': str(compound.subclass.id),
            'name': compound.subclass.name
        },
        'treatments': [
            {
                'id': str(treatment.id),
    ...
    try:
        ...
        if subclass_id:
            try:
                ...
                return JsonResponse({
                    'status': 'error',
                    'message': 'Invalid subclass ID format'
                }, status=400)
        elif subclass_name:
        ...
        if origin_id:
            try:
                ...
                return JsonResponse({
                    'status': 'error',
                    'message': 'Invalid origin ID format'
                }, status=400)
...
def metadata_api(request):
    ...
    try:
        ...
        compound_types = Compound.objects.values_list('type', flat=True).distinct().order_by('type')
