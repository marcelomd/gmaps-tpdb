...
def generate_and_save_molecule_image(compound, size=(300, 200), force_regenerate=False):
    ...
    """
    logger.info(
        f"[TRACE] Starting image generation for compound: {compound.name}, SMILE: {compound.smile[:50] if compound.smile else 'None'}"
    )

    if not RDKIT_AVAILABLE:
        logger.warning(f"[TRACE] RDKit not available")
        return False

    if not compound.smile:
        logger.warning(f"[TRACE] No SMILE string for compound {compound.name}")
        return False

    # Skip if image already exists and not forcing regeneration
    if compound.molecule_image and not force_regenerate:
        logger.info(f"[TRACE] Image already exists for {compound.name}, skipping")
        return True

    try:
        logger.info(f"[TRACE] About to parse SMILE for {compound.name}")
        # Parse SMILE string with timeout protection
        mol = Chem.MolFromSmiles(compound.smile)
        logger.info(f"[TRACE] SMILE parsed successfully for {compound.name}")
        ...
        logger.info(f"[TRACE] About to generate image for {compound.name}")
        # Generate the image
        img = Draw.MolToImage(mol, size=size)
        logger.info(f"[TRACE] Image generated for {compound.name}")

        # Save to BytesIO buffer
        buffer = io.BytesIO()
        img.save(buffer, format="PNG")
        buffer.seek(0)
        logger.info(f"[TRACE] Image saved to buffer for {compound.name}")
        ...
        logger.info(f"Generated molecule image for compound: {compound.name}")
        return True
